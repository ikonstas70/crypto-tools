import requests
import time
import shutil
import os
import sys
import tty
import termios
import select  # Import select for non-blocking input

def set_terminal_size(width, height):
    """Sets the terminal size to a fixed width and height."""
    os.system(f"stty cols {width} rows {height}")  # Locks terminal size

def fetch_crypto_price(currency):
    """Fetches the price of a cryptocurrency from Coinbase."""
    url = f"https://api.coinbase.com/v2/prices/{currency}-USD/spot"
    try:
        response = requests.get(url, timeout=5)
        response.raise_for_status()  # Check for HTTP errors
        data = response.json()
        return float(data.get('data', {}).get('amount', 0))  # Extract and convert to float
    except requests.exceptions.RequestException:
        return None  # Return None if fetching fails

def format_price_change(current_price, previous_price):
    """Formats price with color based on change."""
    if current_price is None:
        return "\033[0;33mData unavailable\033[0m"  # Yellow text for unavailable data

    formatted_price = f"{current_price:.2f}"  # Default format

    if previous_price is not None:
        change = current_price - previous_price
        if change > 0:
            formatted_price = f"\033[0;32m{current_price:.2f} (+{change:.2f})\033[0m"  # Green
        elif change < 0:
            formatted_price = f"\033[0;31m{current_price:.2f} ({change:.2f})\033[0m"  # Red

    return formatted_price

def display_crypto_prices(currencies, previous_prices, frame_width):
    """Fetches and displays cryptocurrency prices inside a static frame."""
    os.system('clear' if os.name == 'posix' else 'cls')  # Clear terminal
    print("\033[H", end="")  # Move cursor to top-left to prevent scrolling

    print("═" * frame_width)
    print(" Crypto Prices (USD) ")
    print("═" * frame_width)

    for currency in currencies:
        current_price = fetch_crypto_price(currency)
        formatted_price = format_price_change(current_price, previous_prices.get(currency))
        print(f" {currency}: {formatted_price}")

        if current_price is not None:
            previous_prices[currency] = current_price  # Update previous price

    print("═" * frame_width)
    print(" Press 'Q' to Quit ")

def get_single_char():
    """Reads a single keypress without requiring Enter."""
    fd = sys.stdin.fileno()
    old_settings = termios.tcgetattr(fd)
    try:
        tty.setraw(fd)
        return sys.stdin.read(1)
    finally:
        termios.tcsetattr(fd, termios.TCSADRAIN, old_settings)

def main():
    """Main function to run the price tracker."""
    currencies_to_track = ['BTC', 'ETH', 'LTC', 'DOGE']
    previous_prices = {}

    frame_width = 40  # Fixed width for better formatting
    terminal_width = 36
    terminal_height = 8

    # Lock the terminal size to 36x8
    set_terminal_size(terminal_width, terminal_height)

    try:
        while True:
            display_crypto_prices(currencies_to_track, previous_prices, frame_width)

            # Check if user wants to quit
            if sys.stdin in select.select([sys.stdin], [], [], 10)[0]:  # 10s timeout
                key = get_single_char()
                if key.lower() == 'q':
                    print("\nExiting price tracker. Goodbye!")
                    break
    except KeyboardInterrupt:
        print("\nExiting price tracker. Goodbye!")

if __name__ == "__main__":
    main()
